...

{
  ...
  update()
  {
    ...
    for (i=enemys.length-1; i >= 0; i--)
    {
      ...
      if (length <= 30/ws)
      {
        //if player has collided with anything, and has captured humans,
        //then release them.
        ...
        //if player has collided with a stealer, which has a captured human,
        //then release it.
        if(enemys[i].id == 4 && enemys[i].captured == 2)
        {
          for(j=enemys.length-1; j>=0; j--)
          {
