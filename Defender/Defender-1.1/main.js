...
function draw() {
  ...
  //textSize(25/ws);
  //text(humans.length,100,200);
  //text(spawns.length,100,250);
    ...
    if(temp == true);//if(temp == 9)
    {
      ...
      if(temp == true)
      {
        ...
        for (i=enemys.length-1; i >= 0; i--)
        {
          if(enemys[i].id == 0)
          {
            //yes, probability of turning lander into a stealer (1\1000)
            temp = round(random(0,500));
            if(temp == 499)
            {
              //create a stealer
              enemys.push(new stealer(enemys[i].x,enemys[i].y,i));  //new
              //delete the lander
              enemys.splice(i,1);
              //break out of Loop
              i=enemys.length-1;
  ...
  //CHECK FO LEVEL COMPLETION
  No_Humans();
  Is_Level_Complete();

  //ROUTINE FOR RESPAWN PROBABILITY
  // if(game_state == 1)
  // {
  //   temp = random(4999);
  //   if(temp < 2)
